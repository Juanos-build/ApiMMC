...
namespace ApiMMC.Services.Jobs.Proccess
{
        ...
        ResponseHelper responseHelper) : IJob
    {
        ...
        {
            using (LoggerManager.logger.PushScopeProperty("JobName", nameof(ConsultaJob)))
            {
                _responseHelper.Info($"Inicio proceso consultas");
                ...
                var result = await _readdingService.ConsultarProcceso(progress);
